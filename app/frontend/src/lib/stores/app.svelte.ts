...
export type View = 'chat' | 'approvals' | 'tasks' | 'extensions' | 'usage' | 'settings';
...
class AppState {
  ...
  async respondApproval(id: string, approve: boolean) {
    const r = await this.try('approval/respond', { approvalId: id, approve });
