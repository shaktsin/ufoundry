...
<p class="text-sm text-zinc-400 mb-3">Hidden models don't appear in the model picker. Prices are USD per million tokens and drive the cost numbers in Usage.</p>

{#each byProvider as [prov, list]}
  <div class="flex items-center mt-5 mb-2">
    <h2 class="text-xs font-semibold uppercase tracking-wider text-zinc-500">{names[prov] ?? prov}</h2>
      ...
      <tbody>
        {#each list as m (m.id)}
          ...
          <tr class={m.hidden ? 'opacity-50' : ''}>
            <td><div class="text-zinc-100">{m.displayName || m.id}</div><div class="text-[11px] text-zinc-500 font-mono selectable">{m.id}</div></td>
            <td class="text-[11px] text-zinc-500">{[m.supportsTools && 'tools', m.supportsImages && 'images', m.supportsReasoning && 'thinking'].filter(Boolean).join(' · ')}</td>
            {#if editing === key}
            ...
            {:else}
              <td class="text-right text-zinc-400">{m.inputPerMTok ? `$${m.inputPerMTok}` : '—'}</td>
              <td class="text-right text-zinc-400">{m.cachedInputPerMTok ? `$${m.cachedInputPerMTok}` : '—'}</td>
              <td class="text-right text-zinc-400">{m.outputPerMTok ? `$${m.outputPerMTok}` : '—'}</td>
