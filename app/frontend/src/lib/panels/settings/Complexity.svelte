...
{#if draft}
  <div class="card p-4 mb-4">
    ...
    <p class="text-xs text-zinc-500 mt-2">Auto looks at each message (length, code, words like “research” or “step by step”) and picks Quick, Standard or Deep.</p>
  </div>
    ...
    <table class="data-table">
      ...
      <tbody>
        {#each draft.presets as p}
          <tr>
            <td><div class="text-zinc-100 capitalize">{p.level}</div><div class="text-[11px] text-zinc-500 max-w-56">{describe[p.level] ?? ''}</div></td>
            <td>
