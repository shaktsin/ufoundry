...
{#if data}
  <div class="grid grid-cols-4 gap-3 mb-5">
    {#each [
      ...
      <div class="card p-3"><div class="text-xs text-muted">{label}</div><div class="text-xl font-semibold mt-0.5">{value}</div></div>
    {/each}
  </div>

  {#if budgets.length}
    <h2 class="text-sm font-semibold text-ink-soft mb-2">Monthly budgets</h2>
    <div class="grid grid-cols-2 gap-3 mb-5">
      {#each budgets as c (c.id)}
        {@const spent = c.monthUsage?.costUsd ?? 0}
        {@const pct = Math.min(100, (spent / (c.monthlyBudgetUsd || 1)) * 100)}
        <div class="card p-3">
          <div class="flex text-sm"><span>{c.label}</span><span class="ml-auto text-muted">{fmtUsd(spent)} / {fmtUsd(c.monthlyBudgetUsd)}</span></div>
          <div class="h-1.5 bg-raised rounded mt-2 overflow-hidden">
            <div class="h-full {pct >= 100 ? 'bg-rust' : pct >= 80 ? 'bg-amber-warm' : 'bg-clay'}" style="width: {pct}%"></div>
          </div>
          <div class="text-[11px] text-muted mt-1">{c.hardStop ? 'Stops at 100%' : 'Warns at 80% and 100%'}</div>
        </div>
      {/each}
    </div>
  {/if}
  ...
  {#if data.rows.length === 0}
    <div class="card p-8 text-center text-sm text-muted">No usage in this period.</div>
  {:else}
        ...
        <tbody>
          {#each data.rows as r (r.key)}
            <tr>
              ...
              <td><div class="h-1.5 bg-clay/70 rounded" style="width: {(r.usage.costUsd / maxCost) * 100}%"></div></td>
              <td class="text-right text-muted">{fmtTokens(r.usage.inputTokens)}</td>
              <td class="text-right text-muted">{fmtTokens(r.usage.outputTokens)}</td>
              <td class="text-right text-muted">{r.usage.requests}</td>
    ...
    <p class="text-[11px] text-faint mt-2">Costs use the model prices in Settings → Models. “~” means some requests used estimated token counts or unknown prices.</p>
  {/if}
{/if}
