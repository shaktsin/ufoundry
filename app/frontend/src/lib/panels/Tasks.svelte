...
{#if tasks.length === 0}
  <div class="card p-8 text-center text-sm text-zinc-500">No {filter === 'active' ? 'active ' : ''}tasks. You can also ask in chat: “every weekday at 9, summarize my inbox”.</div>
{:else}
    ...
    <table class="data-table">
      ...
      <tbody>
        {#each tasks as t (t.id)}
          <tr>
            <td class="max-w-xs">
              <div class="text-zinc-100">{t.name}</div>
              <div class="text-xs text-zinc-500 truncate" title={t.prompt}>{t.prompt}</div>
              {#if t.status !== 'active'}<span class="pill bg-zinc-800 text-zinc-400 mt-1">{t.status}</span>{/if}
            </td>
            <td class="text-xs text-zinc-400">{scheduleText(t)}<div class="text-zinc-600">{t.timezone}</div></td>
            <td class="text-xs text-zinc-400">{fmtDateTime(t.nextRunAt)}</td>
            <td class="text-xs">
              <span class="text-zinc-400">{fmtDateTime(t.lastRunAt)}</span>
              {#if t.lastError}<div class="text-red-400 truncate max-w-48" title={t.lastError}>{t.lastError}</div>{/if}
            </td>
          ...
          {#if runsFor === t.id}
            <tr><td colspan="5" class="bg-zinc-950/50">
              {#if runs.length === 0}<p class="text-xs text-zinc-500">No runs yet.</p>{/if}
              {#each runs as r (r.id)}
                <div class="flex gap-3 text-xs py-1">
                  <span class="w-32 text-zinc-500">{fmtDateTime(r.startedAt)}</span>
                  <span class="w-16 {r.status === 'success' ? 'text-emerald-400' : r.status === 'failed' ? 'text-red-400' : 'text-amber-400'}">{r.status}</span>
                  <span class="flex-1 text-zinc-400 truncate selectable" title={r.error || r.result}>{r.error || r.result}</span>
                </div>
              {/each}
            </td></tr>
          {/if}
