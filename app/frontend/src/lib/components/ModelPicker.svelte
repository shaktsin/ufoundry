...
<div class="flex items-center gap-1.5 flex-wrap">
  ...
  <div class="inline-flex rounded-md border border-zinc-700 overflow-hidden" role="group" aria-label="Complexity">
    {#each levels as l}
      {@const on = (value.complexity || '') === l.id}
      {@const inherited = !value.complexity && defaultComplexity === l.id}
      <button
        class="px-2 py-1 text-xs transition-colors {on ? 'bg-violet-600 text-white' : inherited ? 'bg-zinc-800 text-zinc-200' : 'text-zinc-400 hover:bg-zinc-800'}"
