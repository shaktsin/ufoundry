...
from ufoundry.connectors.telegram_format import markdown_to_telegram_html, split_message
from ufoundry.connectors.telegram_resilience import (
    ChatCircuitBreakers,
    TelegramRateLimiter,
    backoff_delay,
)
from ufoundry.config import load_config
from ufoundry.connectors.base import BaseConnector, ConnectorStatus

logger = logging.getLogger("ufoundry.connectors.telegram_bot")
...
class TelegramBotConnector(BaseConnector):
    ...
    def _get(self, method: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        req = Request(url, headers={"User-Agent": "ufoundry/0.1"})
...
def _connector_env_token(connector_name: str) -> Optional[str]:
    ...
    candidates = [
        f"UFOUNDRY_CONNECTOR_{normalized}_TOKEN",
        "UFOUNDRY_TELEGRAM_BOT_TOKEN",
        "UFOUNDRY_TELEGRAM_TOKEN",
    ]
...
def main() -> None:
    ...
    if not ws_token:
        raise SystemExit("WebSocket token not configured (UFOUNDRY_WS_TOKEN)")
