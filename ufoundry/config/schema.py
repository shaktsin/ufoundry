...
class WorkspaceConfig:
    """A named, sandboxed directory ufoundry may operate in.
...
class MCPServerConfig:
    ...
    ``stdio`` (default) — ufoundry spawns the server as a child process and
    communicates via stdin/stdout.  Use for local servers installed via npm/pip.

    ``http`` — the server is already running (e.g. in Docker) and ufoundry
...
@dataclass
class StorageConfig:
    db_path: str = "~/.ufoundry/ufoundry.db"
    vault_dir: str = "~/.ufoundry/vault"


@dataclass
class RuntimeConfig:
    pid_file: str = "~/.ufoundry/ufoundry.pid"
    log_dir: str = "~/.ufoundry/logs"
...
class AgentsConfig:
    ...
    Example config.yaml::
        ...
        agents:
          enabled: true
          context_file: ~/.ufoundry/AGENT.md   # optional — user-defined agent context
          orchestrator:
            provider: openai
            model: o3
    ...
    context_file: str = "~/.ufoundry/AGENT.md"
...
class AgentTeamsConfig:
    ...
    install_dir: str = "~/.ufoundry/agent-teams"
