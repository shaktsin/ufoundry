...
**stdio** (default)
    ufoundry spawns the server as a child process.  Communication uses
...
**http**
    ...
    ufoundry connects over the network using the MCP Streamable HTTP
...
from ufoundry.tools.shell_env import apply_zsh_path

logger = logging.getLogger("ufoundry.tools.mcp")
    ...
    async def _initialize(self, server_name: str, timeout: float = 15.0) -> None:
        result = await self._rpc(
            ...
            "initialize",
            {
                ...
                "capabilities": {},
                "clientInfo": {"name": "ufoundry", "version": "1.0.0"},
            },
            timeout=timeout,
        )
