...
Set the active workspace at the start of each agent/job:
    ...
    from ufoundry.tools.workspace import set_active_workspace, resolve_workspace
...
from ufoundry.config.schema import WorkspaceConfig
...
# Fallback when no workspace is configured: ~/.ufoundry/tmp
_TMP_WORKSPACE = WorkspaceConfig(
    name="tmp",
    path=str(Path.home() / ".ufoundry" / "tmp"),
    default=True,
)
...
def workspace_summary(workspaces: List[WorkspaceConfig]) -> str:
    ...
    if not workspaces:
        return "  tmp  (~/.ufoundry/tmp)  [default, rw, shell]"
    lines = []
    for ws in workspaces:
        acl = ws.acl
        flags = []
