...
async def _shell_run(args: Dict[str, Any]) -> ToolResult:
    ...
    from ufoundry.tools.workspace import get_active_workspace
    ...
    if explicit_cwd:
        ...
        if not candidate.is_absolute() and ws:
            ...
            # never to the gateway process CWD (which is the ufoundry source dir).
    ...
    _logging.getLogger("ufoundry.tools.builtin").debug(
...
async def _file_write(args: Dict[str, Any]) -> ToolResult:
    from ufoundry.tools.workspace import get_active_workspace, enforce_path
