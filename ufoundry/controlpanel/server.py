...
from ufoundry.controlpanel.routers import admin, agent_teams, chat, config_router, connectors, dashboard, logs, policy, skills, tasks
from ufoundry.controlpanel.store import PanelStore

logger = logging.getLogger("ufoundry.controlpanel")
...
def _add_oauth_routes(app: FastAPI) -> None:
    ...
    async def google_oauth_callback(request: Request, code: str = "", state: str = "", error: str = ""):
        ...
        try:
            from ufoundry.tools.google.auth import exchange_code
            exchange_code(
                client_id=client_id,
...
def run_panel(
    ...
    from ufoundry.gateway import build_runtime
