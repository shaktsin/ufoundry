...
Commands:
  ufoundry google setup    — import credentials from client_secret.json or prompt
  ufoundry google login    — open browser OAuth flow, store token locally
  ufoundry google status   — show auth state
  ufoundry google logout   — revoke and delete stored token
"""
...
logger = logging.getLogger("ufoundry.cli.google")
...
def cmd_setup(args) -> None:
    ...
    from ufoundry.config import load_config
    from ufoundry.config.loader import save_config
    ...
    print("\nNext step — run:  ufoundry google login")
...
def run_oauth_login(client_id: str, client_secret: str, db) -> bool:
    ...
    """
    from ufoundry.tools.google.auth import build_auth_url, exchange_code
...
def cmd_login(args) -> None:
    ...
    if not client_id or not client_secret:
        print("❌  Google credentials not configured. Run:  ufoundry google setup")
        return
...
def cmd_status(args) -> None:
    ...
    from ufoundry.config import load_config
    from ufoundry.storage import Database
    from ufoundry.tools.google.auth import is_authorized, get_credentials
    ...
    print("\n─── Google Status ────────────────────────────────────────────────")
    if not client_id or not client_secret:
        print("  Credentials : ❌  not configured  (run: ufoundry google setup)")
        return
    ...
    try:
        if not is_authorized(db):
            print("  Token       : ❌  not stored  (run: ufoundry google login)")
        else:
            ...
            if creds and creds.valid:
                ...
                print("  Token       : ❌  invalid — run: ufoundry google login")
    except ImportError:
