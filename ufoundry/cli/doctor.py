...
def _check_connector_credentials(connector, cfg) -> tuple[str, str]:
    ...
    if conn_type == "telegram_bot":
        ...
        fallback = os.environ.get("UFOUNDRY_TELEGRAM_TOKEN") or getattr(cfg.telegram, "token", None)
        if fallback:
            return ("✓", "Token present (from UFOUNDRY_TELEGRAM_TOKEN)")
        ...
        if token:
            return ("✓", "Token present")
        fallback = os.environ.get("UFOUNDRY_DISCORD_TOKEN") or getattr(cfg.discord, "token", None)
        if fallback:
            return ("✓", "Token present (from UFOUNDRY_DISCORD_TOKEN)")
        return ("✗", "Missing token")
    return ("⚠", "Unknown type")
