...
from ufoundry.config.schema import (
    Config,
    ConnectorConfig,
    WorkspaceACL,
    WorkspaceConfig,
...
from ufoundry.llm.openai_client import _is_reasoning_model
from ufoundry.config.loader import load_config, save_config as save_cfg, store_secrets
from ufoundry.storage import Database
...
def run_wizard(
    ...
    """
    config_file = Path(config_path).expanduser() if config_path else Path.home() / ".ufoundry" / "config.yaml"
    ...
    try:
        ...
        if "workspaces" in selected:
            ...
            console.print(
                "[dim]Named directories ufoundry can read/write. Each has its own ACL.\n"
...
def _step_control_panel(cfg: Config, is_update: bool, out_tokens: dict) -> None:
    ...
    from ufoundry.config.schema import ControlPanelConfig as _CPConfig
    ...
    while True:
        ...
        if action == "web":
            ...
            from ufoundry.cli.control_panel_setup import _get_bot_token_and_discover_chat_id
...
def _setup_google_workspace(cfg: Config) -> None:
    ...
    from ufoundry.cli.google import _parse_client_secret_json, run_oauth_login, _OAUTH_CALLBACK_PORT
    from ufoundry.storage import Database
    ...
    try:
        ...
        if result.returncode != 0:
            console.print(
                ...
                "[yellow]You can install manually: pip install 'ufoundry[google]'[/yellow]"
            )
            return
    ...
    if not do_login:
        console.print(
            "[yellow]Skipping login — run [bold]ufoundry google login[/bold] later to authorise.[/yellow]\n"
        )
        return

    # Save config first so the DB path is resolved
    from ufoundry.config.loader import save_config as _save_cfg
    from pathlib import Path as _Path
    _cfg_path = str(_Path.home() / ".ufoundry" / "config.yaml")
    ...
    if ok:
        ...
        from ufoundry.tools.google.auth import get_credentials
        try:
