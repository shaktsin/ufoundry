...
from ufoundry.config import load_config, parse_override_args
from ufoundry.config.schema import get_connector_role
from ufoundry.security import filter_pii
from ufoundry.control_panel import ControlPanelManager
from ufoundry.models import IncomingMessage
from ufoundry.policy import PolicyEngine
from ufoundry.router import ControlConfig, MessageRouter
from ufoundry.scheduler import TaskScheduler
from ufoundry.skills import SkillRegistry, SkillRuntimeProvisioner
from ufoundry.storage import Database, Queue
from ufoundry.tools import (
    Attachment,
    Tool,
    ToolRegistry,
    ToolResult,
...
from ufoundry.tools.mcp_registry import MCPRegistry
from ufoundry.worker import Worker
from ufoundry.ws import ChannelHub, WebsocketGateway

logger = logging.getLogger("ufoundry.gateway")
...
class Gateway:
    ...
    async def _on_ws_event(self, connector: str, channel: str, mode: str, data: dict) -> None:
        ...
        if connector_role == "listener":
            from ufoundry.controlpanel.connector import PANEL_CHAT_ID, PANEL_CHANNEL, PANEL_CONNECTOR
    ...
    async def _start_ws_gateway(self) -> None:
        ...
        if not token:
            logger.warning("WebSocket gateway disabled (UFOUNDRY_WS_TOKEN not set)")
            return
...
def _resolve_log_level(value: Optional[str]) -> int:
    raw = value or os.environ.get("UFOUNDRY_LOG_LEVEL", "INFO")
