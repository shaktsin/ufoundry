...
logger = logging.getLogger("ufoundry.orchestrator")
...
def _gateway_cmd(config_path: str, log_level: Optional[str]) -> List[str]:
    cmd = [_python_cmd(), "-m", "ufoundry.gateway", "--config", config_path]
    if log_level:
...
def _telegram_bot_cmd(connector: str, config_path: str, log_level: Optional[str]) -> List[str]:
    ...
    cmd = [_python_cmd(), "-m", "ufoundry.connectors.telegram_bot_connector", "--connector", connector, "--config", config_path]
    if log_level:
        cmd.extend(["--log-level", log_level])
    return cmd
...
def _telegram_user_cmd(connector: str, config_path: str, log_level: Optional[str]) -> List[str]:
    ...
    cmd = [_python_cmd(), "-m", "ufoundry.connectors.telegram_user_connector", "--connector", connector, "--config", config_path]
    if log_level:
        cmd.extend(["--log-level", log_level])
    return cmd
...
def _gmail_watch_cmd(connector: str, config_path: str, log_level: Optional[str]) -> List[str]:
    ...
    cmd = [_python_cmd(), "-m", "ufoundry.connectors.gmail_connector", "--connector", connector, "--config", config_path]
    if log_level:
        cmd.extend(["--log-level", log_level])
    return cmd
