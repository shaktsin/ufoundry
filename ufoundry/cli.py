...
from ufoundry.config import load_config, parse_override_args, run_wizard
from ufoundry.skills import SkillRegistry, lint_skill_dir
from ufoundry.skills.loader import load_skill_metadata
from ufoundry.skills.installer import SkillInstaller
from ufoundry.cli.control_panel_setup import run_setup as run_control_panel_setup


def main() -> None:
    parser = argparse.ArgumentParser(prog="ufoundry")
    parser.add_argument("--config", dest="config", default=None)
    parser.add_argument(
        "--log-level",
        dest="log_level",
        default=None,
        help="Logging level (DEBUG, INFO, WARNING, ERROR). Can also use UFOUNDRY_LOG_LEVEL.",
    )
    parser.add_argument(
        "--set",
        dest="overrides",
        action="append",
        default=[],
        help="Override config key (section.field=value or UFOUNDRY_ENV=value).",
    )
...
def _start_daemon(config_path: Optional[str], overrides: list[str], log_level: Optional[str]) -> None:
    ...
    log_file = log_dir / "ufoundry.log"

    cmd = [
        sys.executable,
        "-m",
        "ufoundry.orchestrator",
        "--config",
        str(resolved_path),
        "--log-level",
        log_level or "INFO",
...
def _handle_skills(args) -> None:
    base_dirs = [Path.cwd() / "skills", Path.home() / ".ufoundry" / "skills"]
    skills_dir = base_dirs[1]  # Use ~/.ufoundry/skills as default installation directory
    ...
    if args.skills_command == "list":
        ...
        if not installed:
            ...
            print(f"\nInstall skills with: ufoundry skills install <source>")
            print(f"  - From PyPI: ufoundry skills install ufoundry-skill-github")
            print(f"  - From GitHub: ufoundry skills install https://github.com/user/skill.git")
            print(f"  - From path: ufoundry skills install ./my-skill")
            return
...
def _handle_channels(args) -> None:
    if args.channels_command == "telegram":
        ...
        os.execvpe(sys.executable, [sys.executable, "-m", "ufoundry.connectors.telegram_bot_connector", *telegram_args], os.environ)
        return
    if args.channels_command == "telegram-user":
        ...
        os.execvpe(
            sys.executable,
            [sys.executable, "-m", "ufoundry.connectors.telegram_user_connector", *telegram_args],
            os.environ,
        )
        return
...
def _run_orchestrator(config_path: Optional[str], log_level: Optional[str]) -> None:
    ...
    os.execvpe(sys.executable, [sys.executable, "-m", "ufoundry.orchestrator", *args], os.environ)
